    ...
    def run(self):
        ...
        while (self.do_run == True) & (self.target_block_reached() == False):
            if self.target_block is not None:
                ...
                self.logger.info('Pending transactions = {} / Queued transactions = {} / Target block = {} / Remaining blocks = {}'.format(int(self.web3.txpool.status.pending, 16), len(tx_queue), self.target_block, self.target_block - self.web3.eth.blockNumber))
            else:
                self.logger.debug('Current block = {}'.format(self.web3.eth.blockNumber))
                self.logger.info('Pending transactions = {} / Queued transactions = {}'.format(int(self.web3.txpool.status.pending, 16), len(tx_queue)))
            time.sleep(10)
